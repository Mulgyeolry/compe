...
func TestChatParamsRequestBodyDisablesThinking(t *testing.T) {
	...
	content := `{"schema_version":"competition-audit-v11","document_type":"listing","source_role":"official_primary","computer_related":false,"competition_announcement":false,"rejection_reason":"列表页"}`
...
func TestClassifyRetriesOnceOnEmptyContentThenSucceeds(t *testing.T) {
	...
	valid := `{"schema_version":"competition-audit-v11","document_type":"official_announcement","source_role":"official_primary","computer_related":true,"competition_announcement":true,"rejection_reason":""}`
...
func TestEnrichSegmentRetriesOnceOnEmptyContentThenSucceeds(t *testing.T) {
	...
	valid := `{"schema_version":"competition-audit-v11","identity":{"organizer":{"value":"主办方","evidence":"主办方：组委会","edition":"2026","confidence":"high"}},"facts":{},"events":[]}`
	var calls atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		if calls.Add(1) == 1 {
