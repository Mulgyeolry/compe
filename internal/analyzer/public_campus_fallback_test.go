...
func TestHuaweiAnalyzeUsesFallbackWithoutEnrich(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		_, _ = w.Write([]byte(chatCompletionResponse(`{"schema_version":"competition-audit-v11","document_type":"campus_internal","source_role":"campus_forwarding","computer_related":false,"competition_announcement":false,"rejection_reason":"校内转发通知，非官方主办方发布的有效公告，且数学建模竞赛非计算机领域"}`)))
	}))
	defer server.Close()
	t.Setenv("OPENAI_BASE_URL", server.URL+"/v1")
	t.Setenv("OPENAI_API_KEY", "test-key")
