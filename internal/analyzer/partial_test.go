...
func TestEnrichSegmentFailureDoesNotCancelSiblings(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(string(body), "segment-b") {
			...
			_, _ = w.Write([]byte(chatCompletionResponse(`{"schema_version":"competition-audit-v9","identity":{"organizer":{"value":"中国计算机学会","evidence":"主办方：中国计算机学会","edition":"2026","confidence":"high"}},"facts":{"registration_end":{"value":"2026年9月20日","evidence":"报名截止时间为2026年9月20日","edition":"2026","confidence":"high"}},"events":[{"type":"registration_opened","evidence":"本赛事现已开放报名","edition":"2026","confidence":"high"}]}`)))
			return
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(chatCompletionResponse(`{"schema_version":"competition-audit-v9"}`)))
	}))
	defer server.Close()
...
func TestEnrichInsufficientSuccessReturnsPlainError(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(string(body), "segment-good") {
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(chatCompletionResponse(`{"schema_version":"competition-audit-v9","identity":{"organizer":{"value":"主办方","evidence":"主办方：主办方","edition":"2026","confidence":"high"}}}`)))
			return
		}
...
func TestAnalyzeRetainsStableFieldsOnPartialExtraction(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(chatCompletionResponse(`{"schema_version":"competition-audit-v9","identity":{"edition":{"value":"2026","evidence":"2026全国大学生程序设计大赛","edition":"2026","confidence":"high"},"organizer":{"value":"中国计算机学会","evidence":"主办方：中国计算机学会","edition":"2026","confidence":"high"}},"facts":{"fee":{"value":"50元/人","evidence":"报名费为50元/人","edition":"2026","confidence":"high"}}}`)))
	}))
	defer server.Close()
	t.Setenv("OPENAI_BASE_URL", server.URL+"/v1")
	t.Setenv("OPENAI_API_KEY", "test-key")
