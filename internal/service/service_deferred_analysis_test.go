...
func TestPendingAnalysisPreservesExistingCanonicalData(t *testing.T) {
	...
	if !strings.Contains(audit, "competition-audit-v12") {
...
func TestSuccessfulUpgradeStillClearsInvalidatedFacts(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		if calls.Add(1) == 1 {
			...
			_, _ = w.Write([]byte(chatCompletionTestResponse(`{"schema_version":"competition-audit-v12","document_type":"official_announcement","source_role":"official_primary","computer_related":true,"competition_announcement":true,"rejection_reason":""}`)))
			return
		}
		// Subsequent calls are the enrichment pass; they omit the stale v6
		// registration end so a successful v7 upgrade may clear it.
		_, _ = w.Write([]byte(chatCompletionTestResponse(`{"schema_version":"competition-audit-v12","identity":{"organizer":{"value":"中国研究生数学建模竞赛组织委员会","evidence":"主办方：中国研究生数学建模竞赛组织委员会","edition":"2026","confidence":"high"}},"facts":{"registration_start":{"value":"2026年6月1日8:00","evidence":"报名时间：2026年6月1日8:00","edition":"2026","confidence":"high"}},"events":[{"type":"registration_opened","evidence":"本赛事现已开放报名","edition":"2026","confidence":"high"}]}`)))
	}))
	defer server.Close()
	t.Setenv("OPENAI_BASE_URL", server.URL+"/v1")
	t.Setenv("OPENAI_API_KEY", "test-key")
